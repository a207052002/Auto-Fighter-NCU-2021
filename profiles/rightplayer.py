...
def combatLogic(enemy, me, eventmap):
    ...
    # eventmap 是一個跟場地大小等長的陣列(list)，每一個位置都是 0-4 的整數，分別代表上面的道具
    # 移動到該格會自動撿起道具
    # 以下為陣列中的數字的意義
    # 0 代表沒有道具在那一格
    # 1 代表存在 P (Power shot) 撿到後可以一次性開啟無視防禦 35% - 65%(隨角色攻擊提高) 生命的無限距離大招
    # 撿到後透過 return 整數：1 來讓角色使用
    # 2 代表存在 A (Avoid) 撿到後可以一次性開啟一個 buff 持續三回合，期間無條件閃避所有攻擊
    # 以上兩個道具撿到後會存在身上直到使用，已經持有一樣的道具可以再撿但是不會額外增加數量
    # 3 代表存在 H (Heart) 撿到後可以回覆 15% 的生命(對坦克型角色有很好的效果)
